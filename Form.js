...
export default function Form() {
    ...
    const handleSubmit=(e)=>{
        e.preventDefault();
        if(pid===""){
            axios.postt('http://localhost:3009/products',productData).then((res)=>{
            ...
            axios.put(`http://localhost:3009/products/${pid}`,productData).then((res)=>{
            alert("successfully submitted");
            fetchData();
            setId('');
            setTitle('');
    ...
    const fetchData=()=>{
            axios.get('http://localhost:3009/products').then((res)=>{setProductList(res.data)})
    ...
    const handleDelete=(id)=>{
        axios.delete(`http://localhost:3009/products/${id}`).then((res)=>{
